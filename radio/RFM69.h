...
#define RF69_MAX_DATA_LEN       255 // to take advantage of the built in AES/CRC we want to limit the frame size to the internal FIFO size (66 bytes - 3 bytes overhead - 2 bytes crc)
#define RF69_SPI_CS             RF_SS_pin // SS is the SPI slave select pin, for instance D10 on ATmega328
...
#define RF69_IRQ_PIN	RF_DIO0_pin
#define RF69_IRQ_NUM	0
...
class RFM69 {
  public:
    ...
    RFM69(uint8_t slaveSelectPin=RF69_SPI_CS, uint8_t interruptPin=RF69_IRQ_PIN, bool isRFM69HW=true, uint8_t interruptNum=RF69_IRQ_NUM) {
      _slaveSelectPin = slaveSelectPin;
      _interruptPin = interruptPin;
      _interruptNum = interruptNum;
      _mode = RF69_MODE_STANDBY;
      _promiscuousMode = true;
      _powerLevel = 31;
    ...
    bool sendWithRetry(uint8_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries=2, uint16_t retryWaitTime=40); // 40ms roundtrip req for 61byte packets
    bool receiveDone();
